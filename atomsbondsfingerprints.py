...
np.save("f:/Users/Montague/Desktop/DStuff/atomsbondsfingerprintsresult", np.array(test_raw_cock_list))
...
for i in range(5):
    class roc_callback_noval(Callback):
        def __init__(self):
            self.x = [imgs[i], fingers[i], maccs[i]]
            self.y = ys[i]
            self.x_tes = [testimgs[i], testfingers[i], testmaccs[i]]
            self.y_tes = testys[i]
    ...
    inm = Input(shape = (167,1, 1))
    xm = keras.layers.Reshape((167,))(inm)
    xm = Dense(5, activation = 'relu')(xm)
    outm = Dense(1, activation = 'relu')(xm)
    ...
    x = keras.layers.GlobalAveragePooling2D()(x)
    x = keras.layers.concatenate([x, outm, outf])
    out = Dense(1, activation='linear')(x)
    
    model = Model(inputs=[input_img, inf, inm], outputs=[out])
    ...
    g = generator.flow(imgs[i], ys[i], batch_size=batch_size, shuffle=True, seed=seed)
    gg = stringgenerator.flow(fingers[i], ys[i], batch_size=batch_size, shuffle=True, seed=seed)
    ggg = stringgenerator.flow(maccs[i], ys[i], batch_size=batch_size, shuffle=True, seed=seed)
    
    def combinegenerator(gen1, gen2, gen3):
        while True:
            x1 = next(gen1)
            x2 = next(gen2)
            x3 = next(gen3)
    ...
    history = model.fit_generator(gggg,
                                  steps_per_epoch=len(imgs[i])//batch_size,
                                  epochs=epochs,
                                  validation_data=([testimgs[i], testfingers[i], testmaccs[i]], testys[i]),
                                   callbacks=[reduce_lr, rocauccalc])


np.save("f:/Users/Montague/Desktop/DStuff/atomsbondsfingerprintsmaccsresult", np.array(test_raw_cock_list))
